...
def cross_correlate_einsum(t,y,data):
    ...
    if len(shape(t))==1 and len(shape(y))==1:
        [t2,y2]=meshgrid(t,y)
    nt=len(t2[0,:]);ny=len(y2[:,0])
    wl_mn=abs(gradient(y2)[0]).min()/1.
    tau_mn=(gradient(t2)[1]).min()/2.
    tmx=[];ymx=[];cmx=[];vmx=[];wl=[];tau=[];vel=[]
    tmx2=zeros((ny,nt));ymx2=zeros((ny,nt));cmx2=zeros((ny,nt));
    wl2=zeros((ny,nt));tau2=zeros((ny,nt));vel2=zeros((ny,nt));
    ...
    for i in range (ny):
        ...
        if len(imx)>1:
            tmx.append(t2[i,imx]);ymx.append(y2[i,imx])
            cmx.append(abs(cin[i,imx]))
            tau.append(2.*gradient(t2[i,imx]))
            tmx2[i,imx]=t2[i,imx];ymx2[i,imx]=y2[i,imx];
#            cmx2[i,imx]=abs(cin[i,imx])
            tau2[i,imx]=2.*abs(gradient(t2[i,imx]));
    
    for i in range (nt):
        imx=find_peaks(cin[:,i])[:,0]
        if len(imx)>1:
            cmx.append(abs(cin[imx,i]))
            wl.append(2*gradient(y2[imx,i]))
            tmx2[imx,i]=t2[imx,i];ymx2[imx,i]=y2[imx,i];
            cmx2[imx,i]=abs(cin[imx,i])
            wl2[imx,i]=2*abs(gradient(y2[imx,i]))
            
    vel2=wl2/tau2    
#    tmx2=tmx2[abs(wl2) >= wl_min]
#    ymx2=ymx2[abs(wl2) >= wl_min]
#    cmx2=cmx2[abs(wl2) >= wl_min]
#    tau2=tau2[abs(wl2) >= wl_min]
#    vel2=vel2[abs(wl2) >= wl_min]
#    wl2=wl2[abs(wl2) >= wl_min]
#    tmx2=tmx2[tau2 > tau_min]
#    ymx2=ymx2[tau2 > tau_min]
#    cmx2=cmx2[tau2 > tau_min]
#    vel2=vel2[tau2 > tau_min]
#    wl2=wl2[tau2 > tau_min]
#    tau2=tau2[tau2 > tau_min]
    return (tmx2,ymx2,cmx2,wl2,tau2,vel2)
...
def wavelet(iw,t,data,n_mode): # t is in minutes
    def wave_ones():
        ...
        im1=i-1;ip1=i+1
        if i==0:im1=0
        if i==n_mode-1:ip1=n_mode-1
        wv_letm=ones((int(modes[im1]),))/int(modes[im1]);
        wv_letp=ones((int(modes[ip1]),))/int(modes[ip1]);
        a=np.convolve(data,wv_letm,mode='same')
        b=np.convolve(data,wv_letp,mode='same')
        pwr=a-b#(data-convolve_al(data,win))
        return pwr
    def wave_morlet():
        sigma=5.
        ti=linspace(-n_mode,n_mode,nt,False)
        cs= (1 + np.exp(-sigma ** 2) - 2 * np.exp(-.75 * sigma ** 2)) ** (-.5)
        ks = np.exp(-.5 * sigma ** 2)
        scale=(n_mode/32.)*dt*modes[i]
        t_mode=ti/scale
        wv_lets=np.exp(-.5 * t_mode ** 2) * \
            (np.exp(1j* sigma * t_mode) - ks) * cs * np.pi**(-.25)
        pwr=np.convolve(data,real(wv_lets),mode='same')
        return (pwr,scale)
    def wave_hat():
...
#    n_mode=len(modes);n_mode=min(n_mode,1024)
    ...
    for i in range (n_mode):
        if iw==0:pwr=wave_ones();data=data-pwr;pd=pd_fft[i]
        if iw==1:f=wave_morlet();pwr=f[0];pd=f[1]
        if iw==2:pwr=wave_hat();pd=pd_fft[i]
