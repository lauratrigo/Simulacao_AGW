...
class GEOSTIDSSimulator:
    ...
    def _build_source(self) -> Source:
        ...
        amplitude_factor = 50  # Mude este valor: 1, 10, 50, 100, 500
    ...
    def seismic_source(self, t: float, lat: np.ndarray, lon: np.ndarray) -> np.ndarray:
        ...
        Args:
            t: tempo atual (s)
            lat: array de latitudes (graus)
            lon: array de longitudes (graus)
        ...
        mask = (t >= arrival_time) & (t < arrival_time + 3 * self.cfg.seismic_period)
        
        # Amplitude decai com a distância
        amplitude = self.cfg.seismic_amplitude / (1 + distance / 100000)
        
        # Forma da onda
        wave = np.sin(omega * (t - arrival_time)) * np.exp(-((t - arrival_time) / self.cfg.seismic_period)**2)
    ...
    def step(self, i_t: int) -> dict:
        ...
        # Calcula fonte sísmica se necessário
        if self.cfg.source_type == "seismic":
            lat_2d, lon_2d = np.meshgrid(g.lat, g.lon, indexing='ij')
            seismic_amp = self.seismic_source(self.time[i_t], lat_2d, lon_2d)
            source_3d = seismic_amp[None, :, :] * np.ones((g.np_, 1, 1))
            source_3d = source_3d * self.source.mask_bound
