...
i_mag=float(input('MAGNETIC OR SPHERICAL, 0 FOR SPHERICAL '))   #1 for magnetic, 0 for spherical
i_inert=float(input('INERTIA OR COLLISIONAL, 0 FOR COLLISIONAL '))   #1 for INERTIA, 0 for COLLISIONAL
...
def campo_ele_mag_coord(t_e, wp_g, wf_g, wq_g, den_t, pot):
    ...
    # mobilidade
    f = mobility()
    mu_p_i = f[0]; mu_h_i = f[1]
    mu_p_e = f[3]; mu_h_e = f[4]
