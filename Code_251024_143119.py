...
time_data = load("time.npy")

# ===== PADDING =====
pad = 250

dados_pad = np.pad(dados_y, ((0, pad), (0, 0), (0, 0)), mode='constant')

dt = time_data[1] - time_data[0]
time_pad = np.concatenate([
    time_data,
    time_data[-1] + dt * np.arange(1, pad + 1)
])
# ===================
...
# im = ax.imshow(100*dados_pad[:, 20, :], origin='lower', cmap='seismic', extent=[time_pad[0], time_pad[-1], y.min(), y.max()])

ix = dados_pad.shape[1] // 2   # posição central
largura = 20                    # número de pontos pra cada lado

fatia = dados_pad[:, ix-largura:ix+largura, :]
fatia_media = fatia.mean(axis=1)

# im = ax.imshow(100*fatia_media,
#                origin='lower',
#                cmap='seismic',
#                extent=[time_pad[0], time_pad[-1], y.min(), y.max()],
#                aspect='auto')
...
# ax.set_ylabel("Altitude (km)")
# ax.set_title("Propagação de AGWs - Arraste para navegar")
...
# except Exception:
#     print("Use a ferramenta de 'mão' na barra inferior para arrastar.")

# # FORÇA A JANELA A FICAR ABERTA:
# show(block=True)

# 1. Prepare os dados básicos
plt.ioff() # Desliga modo interativo temporariamente para estabilizar os sliders
data_plot_base = 100 * fatia_media

# Calcule limites iniciais razoáveis (mas não perfeitos, vamos ajustar nos sliders)
# v_max_abs = np.max(np.abs(data_plot_base))
# v_max_init = v_max_abs
# v_min_init = -v_max_abs

# Em vez de usar o absoluto (simétrico), vamos usar o min e max reais dos dados
v_min_init = np.min(data_plot_base) 
v_max_init = np.max(data_plot_base)

# Para o slider não travar, definimos um limite de manobra baseado no maior valor
v_range_max = np.max(np.abs(data_plot_base))

# 2. Configure a Figura e o Gráfico Principal
# Aumentamos a figura para caber os sliders embaixo
fig = plt.figure(12, figsize=(10, 10)) 
# Reserva espaço inferior para os sliders (left, bottom, width, height)
ax = fig.add_axes([0.1, 0.25, 0.8, 0.7]) 

im = ax.imshow(data_plot_base,
               origin='lower',
               cmap='seismic', # Mantendo a paleta
               vmin=v_min_init, 
               vmax=v_max_init,
               extent=[time_pad[0], time_pad[-1], y.min(), y.max()],
               aspect='auto',
               interpolation="spline36") # Alta resolução visual

ax.set_xlabel("Tempo (min)")
ax.set_ylabel("Altitude (km)")
ax.set_title("Propagação de AGWs")

# Adiciona Colorbar
cb = fig.colorbar(im, ax=ax, pad=0.02)
cb.set_label('Amplitude da Perturbação')

# 3. CONFIGURE OS SLIDERS (CONTROLES)

# Cores dos sliders
axcolor = 'lightgoldenrodyellow'

# --- Slider de CONTRASTE (Faixa Dinâmica) ---
# Define a largura da janela de cores ao redor do centro.
# ax_contrast = fig.add_axes([0.15, 0.15, 0.65, 0.03], facecolor=axcolor)
# s_contrast = Slider(ax_contrast, 'Contraste', 0.1, np.max(np.abs(data_plot_base))*2.0, 
#                     valinit=v_max_init, valfmt='%.1f')

# Slider de CONTRASTE (agora focado na amplitude total)
ax_contrast = fig.add_axes([0.15, 0.15, 0.65, 0.03], facecolor=axcolor)
s_contrast = Slider(ax_contrast, 'Contraste', 0.1, v_range_max * 2.0, 
                    valinit=(v_max_init - v_min_init)/2, valfmt='%.1f')

# --- Slider de BRILHO (Deslocamento do Centro/Bias) ---
# Move o ponto neutro (branco) para cima ou para baixo.
ax_brightness = fig.add_axes([0.15, 0.10, 0.65, 0.03], facecolor=axcolor)
s_brightness = Slider(ax_brightness, 'Brilho (Bias)', -v_max_init, v_max_init, 
                      valinit=0.0, valfmt='%.1f')

# --- Slider de "SATURAÇÃO" (Intensidade do Mapa de Cores) ---
# Técnicamente altera a exposição/ganho da paleta.
ax_saturation = fig.add_axes([0.15, 0.05, 0.65, 0.03], facecolor=axcolor)
s_saturation = Slider(ax_saturation, 'Gama/Saturação', 0.1, 5.0, 
                      valinit=1.0, valfmt='%.1f')


# 4. FUNÇÃO DE ATUALIZAÇÃO (A Lógica)
def update(val):
    # Pega os valores atuais dos sliders
    contrast = s_contrast.val
    brightness = s_brightness.val
    saturation = s_saturation.val # Usaremos para uma pré-normalização gâmica

    # Calcula os novos limites vmin/vmax baseados no contraste e brilho
    new_vmax = brightness + contrast
    new_vmin = brightness - contrast

    # Aplica os novos limites à imagem
    im.set_clim(vmin=new_vmin, vmax=new_vmax)
    
    # Lógica de "Saturação" (Gama): Ajusta a matriz de dados antes de plotar
    # para tornar cores médias mais fortes ou fracas.
    # Nota: Isso é computacionalmente mais pesado que set_clim.
    if saturation != 1.0:
        # Normaliza temporariamente entre 0 e 1, aplica gama, e volta
        data_gama = np.copy(data_plot_base)
        # Ajuste simples de ganho para simular saturação no seismic
        im.set_array(data_gama * saturation)
    else:
        im.set_array(data_plot_base)

    fig.canvas.draw_idle() # Redesenha a figura de forma eficiente

# Vincula a função de atualização às mudanças nos sliders
s_contrast.on_changed(update)
s_brightness.on_changed(update)
s_saturation.on_changed(update)

# 5. ATIVA FERRAMENTAS INTERATIVAS E MOSTRA
try:
    fig.canvas.manager.toolbar.pan() # Ativa arrastar por padrão
except Exception:
    pass

print("Gráfico gerado com controles. Ajuste os sliders embaixo para calibrar a visão.")
plt.show(block=True) # Impede que a janela feche
