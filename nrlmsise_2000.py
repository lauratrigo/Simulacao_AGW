...
def nrl_msis(doy,ut_sec,lst,f107A,f107,ap,glat,glon,dr,alt_o,n_alt):
    output = [nrlmsise_output() for _ in range(n_alt)]
    Input = [nrlmsise_input() for _ in range(n_alt)]
...
#    Input[n_alt-2].ap_a = aph
#    Input[n_alt-1].ap_a = aph
    ...
    #/* evaluate 15 and 16 */
#    flags.switches[9] = -1
#    for i in range(n_alt-2, n_alt):
#        gtd7(Input[i], flags, output[i])

    alt=[];tn=[];den_o=[];den_n=[];den_o2=[];den_n2=[];mass_den=[]
    for i in range(n_alt):
#        print(Input[i].alt,'%E ' %output[i].t[0],'%E ' %output[i].t[1],\
#        '%E ' %output[i].d[1],'%E ' %output[i].d[2],'%E ' %output[i].d[3],\
#        '%E ' %output[i].d[4],)
        alt=append(alt,Input[i].alt)
        tn=append(tn,output[i].t[1])
        den_o=append(den_o,output[i].d[1])
        den_n=append(den_n,output[i].d[7])
        den_o2=append(den_o2,output[i].d[3])
        den_n2=append(den_n2,output[i].d[2])
        mass_den=append(mass_den,output[i].d[5])
    return (alt,tn,den_o,den_n,den_o2,den_n2,mass_den)
...
This is the external data file used by the main program
/* -------------------------------------------------------------------- */
/* ---------  N R L M S I S E - 0 0    M O D E L    2 0 0 1  ---------- */
/* -------------------------------------------------------------------- */
...
#/* S PARAM  */
ps = [9.56827E-01, 6.20637E-02, 3.18433E-02, 0.00000E+00, 0.00000E+00,
...
#/* SEMIANNUAL MULT SAM */
sam = [1.00000E+00, 1.00000E+00, 1.00000E+00, 1.00000E+00, 1.00000E+00,
